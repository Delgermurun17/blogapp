  ...
  async function loadMore() {
    ...
    try {
      const response = await fetch(
        `https://dev.to/api/articles?page=${page}&per_page=${pageSize}${
        ...
        <div className="flex justify-between max-w-[1230px] mx-auto pt-10">
          <div className="flex gap-5">
            {tags.map((tag) => (
              <div
                key={tag.value}
                className={`cursor-pointer hover:text-[#cea072] text-[#495057] font-bold text-[12px] leading-[25px] duration-200 ${
                  selectedTag === tag.value ? "text-[#D4A373]" : ""
                }`}
                onClick={() => {
        ...
        <div className="max-w-[1216px] mx-auto">
          ...
          {!ended && (
            <div className="flex justify-center py-10">
              <button
                onClick={loadMore}
                className="rounded-md border border-gray-500 text-gray-500 p-[12px_20px] bg-white flex hover:bg-gray-100 duration-200"
              >
                Load More
              </button>
            </div>
