    ...
    <main>
        ...
        <div className="max-w-[800px] py-24 mx-auto flex flex-col gap-8">
            <div className="flex flex-col gap-5">
                ...
                <Link href={`https://dev.to/${article.user.username}`} target="_blank"><div className="flex gap-2 font-medium text-sm text-[#696a75] items-center cursor-pointer"><Image src={article.user.profile_image_90} width={28} height={28} className="rounded-[28px]"/>{article.user.name}</div></Link>
                    <div className="font-normal text-sm text-[#696a75] content-center">{dayjs(article.published_at).locale("mn").format("MMMM D, YYYY")}</div>
                </div>
            </div>
            <div><Image className="rounded-xl cursor-pointer" src={article.social_image} width={800} height={462}/></div>
